...
void parse_form_field_data(struct WebRequest *web_request, struct WebForm *formData, char field_name[], char field_value[])
{
	...
	{
		sprintf(formData->phone, "%s", field_value);
	}
}
...
void parse_request_message_line(char *request_message_line, struct WebRequest *web_request, struct WebForm *formData)
{
	...
	// Initialize Field Name & Field Value as empty strings
	strncpy(field_name, "", sizeof(field_name));
	strncpy(field_value, "", sizeof(field_value));

	// If web_request->http_method is "PUT", then identify the actual content of the resource from the request buffer to create the requested resource
	if (strcmp(web_request->http_method, "PUT") == 0 && strstr(request_message_line, "<") != NULL && strstr(request_message_line, ">") != NULL)
	{
		http_response = create_req_file(web_request, request_message_line); // Create the requested resource and set the appropirate response
	}
	...
	if (strcmp(field_name, "GET") == 0 || strcmp(field_name, "POST") == 0 || strcmp(field_name, "HEAD") == 0 || strcmp(field_name, "TRACE") == 0)
	{
		...
		query_str_ptr = strchr(web_request->file, needle);
	...
	else if (strcmp(field_name, "PUT") == 0)
	{
		...
		// delete the previously existing file which has the same name as the requested file in order to create its updated version
...
void parse_request(struct WebRequest *web_request, char *request_buffer, struct WebForm *formData)
{
	...
	// Initialize the WebForm structure elements as empty strings
	...
	strncpy(formData->phone, "", sizeof(formData->phone));

	// Initialize the WebRequest structure elements as empty strings
