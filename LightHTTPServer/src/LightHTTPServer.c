...
char trace_200_ok_res_hdr_bdy[] = "Connection: close\r\n"
...
void send_header()
{
	...
	else if (http_response == HTTP_RESPONSE_TRACE)
	{
		strcat(trace_200_ok_res_hdr_bdy, buffer);
		construct_response_header(default_200_ok_status_line, trace_200_ok_res_hdr_bdy);
		send_data(sizeof(response_header), response_header);
	}
}
	...
	for (;;) // infinite loop
	{
		...
		if ((childpid = fork()) == 0) // creating a child process
		{
			...
			for (;;)
			{
				...
				char req_res_path[200];
				sprintf(req_res_path, WEB_ROOT_DIR "%s", new_request.req_file);

				if (is_directory(req_res_path))
				{
					// check for the index file: index.html
					fp = check_dir_index_file(req_res_path);

					if (fp != NULL)
					{
						strcpy(new_request.req_file, "index.html");
					}
					...
					else
					{
						...
						get_directory_files(req_res_path);
				...
				else
				{
					...
					fp = get_requested_file_pointer(&new_request, new_request.req_file, req_res_path);
				}
